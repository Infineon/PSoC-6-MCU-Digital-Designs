...
* Component using a polling method. The I2C slave accepts a packet from the master with a command
...
*   3. Initializes PWM components to control RGB LED. If initialization of PWM 
*      component fails, system will be in infinite loop.
*   4. I2C slave component receives packets from master and configured the PWM 
*      to drive RGB LED.
...
static void pollI2CwriteBuffer(void)
{
    if( 0UL != (Cy_SCB_I2C_SlaveGetStatus(sI2C_HW, &sI2C_context) &  CY_SCB_I2C_SLAVE_WR_CMPLT))
    {
              
        if( 0UL == (Cy_SCB_I2C_SlaveGetStatus(sI2C_HW, &sI2C_context) &  CY_SCB_I2C_SLAVE_BUS_ERR))
        {
        ...
        Cy_SCB_I2C_SlaveClearWriteStatus(sI2C_HW, &sI2C_context); 
        Cy_SCB_I2C_SlaveConfigWriteBuf(sI2C_HW, i2cWriteBuffer, SL_WR_BUFFER_SIZE, &sI2C_context);    
    }   
    
     /* Check read complete event. */
    if(0UL != (Cy_SCB_I2C_SlaveGetStatus(sI2C_HW, &sI2C_context) &  CY_SCB_I2C_SLAVE_RD_CMPLT))
    {
        /* Configure read buffer for next read. */
         /* Clear read status flags. */       
        Cy_SCB_I2C_SlaveClearReadStatus(sI2C_HW, &sI2C_context);
