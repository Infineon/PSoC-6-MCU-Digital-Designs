...
* (SCB mode) component in master mode using low level PDL function. This example  
* uses polling method to complete the SPI transfers. The SPI master sends the packet 
* with  a command to the SPI slave to control the RGB LED color. The packet with a 
* status is read back by master. CM4 acts as a master and sends various command 
* packets to change the color of RGB LED on the slave. CM0 acts as a slave (Note: Slave 
* on CM0 is developed using low level PDL functions).
*
