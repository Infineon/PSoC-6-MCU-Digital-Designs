...
* SCB mode) component in slave mode using low level PDL function. This example uses 
* polling method to complete the SPI transfers. The SPI master sends the packet with 
* a command to the SPI slave to control the RGB LED color. The packet with a 
* status is read back by master. CM0 acts as a slave. CM4 acts as a master and 
...
*   4. SPI slave component receives packets from master using polling method and low level PDL functions
