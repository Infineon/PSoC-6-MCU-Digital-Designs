...
int main(void)
{
    ...
    /* Configure two PWM for generating quadrature encoded signals. If PWM_phiA
    starts before PWM_phiB clockwise rotation is emulated and if PWM_phiB starts 
    before PWM_phiA then anti-clockwise rotation is emulated */ 
    /* Initialize with config set in component and enable both the PWMs*/
    Cy_TCPWM_PWM_Init(PWM_phiA_HW, PWM_phiA_CNT_NUM, &PWM_phiA_config);
    Cy_TCPWM_PWM_Init(PWM_phiB_HW, PWM_phiB_CNT_NUM, &PWM_phiB_config);
    Cy_TCPWM_Enable_Multiple(PWM_phiA_HW, PWM_phiA_CNT_MASK | PWM_phiB_CNT_MASK);
    
    /* Start both the PWM Components */
    /* Frequency of PWM_phiA and PWM_phiB are 250 Hz (period = 4ms), and for 90 degrees
     phase shift we require 1ms delay */
    if (emulated_rotation == CLOCKWISE)
    {
        Cy_TCPWM_TriggerStart(PWM_phiA_HW, PWM_phiA_CNT_MASK);
        Cy_SysLib_Delay(1); /* 1 msec delay */
        Cy_TCPWM_TriggerStart(PWM_phiB_HW, PWM_phiB_CNT_MASK);
    }
    else if (emulated_rotation == ANTI_CLOCKWISE)
    {
        Cy_TCPWM_TriggerStart(PWM_phiB_HW, PWM_phiB_CNT_MASK);
        Cy_SysLib_Delay(1); /* 1 msec delay */
        Cy_TCPWM_TriggerStart(PWM_phiA_HW, PWM_phiA_CNT_MASK); 
